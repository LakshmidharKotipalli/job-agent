...
class WellfoundScraper(BaseScraper):
    ...
    def search(self, keywords: List[str], locations: List[str], max_results: int = 20) -> List[JobPost]:
        ...
        for keyword in keywords:
            ...
            try:
                ...
                job_cards = self._page.locator('div.styles_jobListing__SIwvo').all()
                ...
                for card in job_cards:
                    ...
                    try:
                        title_el = card.locator('a.styles_jobTitle__Fjsdd')
                        title = title_el.inner_text().strip() if title_el.count() > 0 else "Unknown"

                        company_el = card.locator('a.styles_companyName__mSAgE')
                        company = company_el.inner_text().strip() if company_el.count() > 0 else "Unknown"

                        location_el = card.locator('span.styles_location__nkB1V')
                        ...
                        job_url = f"https://wellfound.com{href}" if href and not href.startswith("http") else href
    ...
    def fetch_description(self, url: str) -> str:
        """Fetch the full description for a specific Wellfound job, reusing the open browser."""
        self._ensure_browser()
        try:
            self._page.goto(url, wait_until="domcontentloaded", timeout=15000)
            time.sleep(2)
            desc_el = self._page.locator('div.styles_description__7MiSm')
            if desc_el.count() > 0:
                return desc_el.inner_text()
            # Fallback to broader selector
            desc_el = self._page.locator('div[data-test="job-description"]')
            if desc_el.count() > 0:
                return desc_el.inner_text()
        except Exception as e:
